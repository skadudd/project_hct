    ...
    (
      ...
      where 1=1
        ...
        and member_no not in (
          select cast(member_no as string)
          from ballosodeuk.ynam.whitelist
        )
        )
...
,df as 
  (select 
    a.register_dt, a.user_id, member_no, gender,age, pre_shoji, current_shoji, first_pay_amt
    ...
    ,first_sub_pay_amt, c.amt as burnt, d.amt as exchange, d.amt * 2 as exchange_cash_rate,
    round(first_sub_pay_amt / first_pay_amt,2) as sub_power, coalesce(round(post_shoji_1 / Nullif(first_sub_pay_amt,0),2),0) as left_behind_multiple
  from merged a
