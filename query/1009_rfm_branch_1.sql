...
,raw_enhanced_external As ( # <- raw에 외부 매출 concat (INL, 다이나믹)
  select 
    ...
    campaignType, campaignName, commission, Event_Value, Null as Rev_Dynamic, Null as Rev_Launcher_Total
  from 
    raw
  union all # 쿠팡 - 다이나믹 추가
  select 
    Null as Airbridge_Device_ID, User_ID, Null as Platform, Null as cum_reward, Null as reg_dttm, Event_Date, 
    Event_Category, Event_Label, Event_Action, Null as campaignType, Null as campaignName, Null as commission, Null as Event_Value, Total_Revenue as Rev_Dynamic, Null as Rev_Launcher_Total
  from 
    dynamic
  ...
  select
    ...
    Null as campaignType, Null as campaignName, Null as commission, Null as Event_Value, Null as Rev_Dynamic, Total_Revenue as Rev_Launcher_Total
  from
    order_complete
)
...
,raw_enhanced_userpropertries as (  
  select 
    ...
    sum(Event_Value) as Event_Value, sum(Rev_Dynamic) as Rev_Dynamic, sum(Rev_Launcher_Total) as Rev_Launcher_Total
  from
    (
      select
        case
...
,agg_table as (
  select 
    ...
    # 총 매출원 sum
    round((COALESCE(SUM(Rev_Offer_Pincrux), 0) + 
     COALESCE(SUM(Rev_Offer_etc), 0) + 
     COALESCE(SUM(Rev_Launcher_Total), 0) + 
     COALESCE(SUM(Rev_Dynamic), 0) + 
     COALESCE(SUM(Rev_Network), 0)),2) AS Rev_Total #매출 값 합,
  ...
  from
    ...
    from
      ...
      select # raw 및 내재 오퍼월 매출 정리
        ...
        count(*) as Event_Count, sum(commission) as Rev_Offer_Pincrux, sum(Rev_Dynamic) as Rev_Dynamic, sum(Rev_Launcher_Total) as Rev_Launcher_Total,
        case when Event_Category = "Order Complete (App)" and Event_Label = "바로가기" then coalesce(sum(Rev_Launcher_Total),0) END as Rev_INL_Launcher,
        case when Event_Category = "Order Complete (App)" and Event_Label = "퀴즈쿠팡" then coalesce(sum(Rev_Launcher_Total),0) END as Rev_INL_Quiz,
        case when Event_Category = "Order Complete (App)" and Event_Label = "챌린지인증쿠팡" then coalesce(sum(Rev_Launcher_Total),0) END as Rev_INL_Challenge,
      from 
        raw_enhanced_userpropertries
      group by 
        Event_Date, User_ID, Platform, Event_Category, Event_Label, Event_Action
      ) raw_e
    left join # 가중 평균 매출 추가
...
-- select sum(Total_Revenue) from order_complete group by Event_Action
...
,base_data AS (
  SELECT 
    ...
    Rev_Offer_Pincrux, Rev_Offer_etc, Rev_INL_Launcher,Rev_INL_Challenge, Rev_INL_Quiz, Rev_Dynamic, Rev_Network,
    Cnt_Offer_Pincrux, Cnt_Rev_Offer_etc, Cnt_Rev_Dynamic, Cnt_Rev_INL_Launcher, Cnt_Rev_INL_Challenge, Cnt_Rev_INL_Quiz, Cnt_Rev_Network
  FROM agg_table
  WHERE Rev_Total > 0
)
...
rfm_features AS (
  SELECT 
    ...
    DATE_DIFF(DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY), MAX(CASE WHEN Count_Offerwall > 0 THEN Event_Date END), DAY) AS Recency_Offerwall,
    DATE_DIFF(DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY), MAX(CASE WHEN Count_Network > 0 THEN Event_Date END), DAY) AS Recency_Network,
    DATE_DIFF(DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY), MAX(CASE WHEN Count_INL_Launcher > 0 THEN Event_Date END), DAY) AS Recency_INL_Launcher,
    DATE_DIFF(DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY), MAX(CASE WHEN Count_INL_Challenge > 0 THEN Event_Date END), DAY) AS Recency_INL_Challenge,
    DATE_DIFF(DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY), MAX(CASE WHEN Count_INL_Quiz > 0 THEN Event_Date END), DAY) AS Recency_INL_Quiz,
    DATE_DIFF(DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY), MAX(CASE WHEN Count_Dynamic > 0 THEN Event_Date END), DAY) AS Recency_Dynamic,
    DATE_DIFF(DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY), MAX(Event_Date), DAY) AS Recncy_Total,
    DATE_DIFF(DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY), 
              MAX(CASE WHEN Count_Offerwall > 0 OR Count_INL_Launcher > 0 OR Count_INL_Challenge > 0 OR Count_INL_Quiz > 0 OR Count_Dynamic > 0 THEN Event_Date END), # <- 새로운 매출원 Input 
              DAY) AS Recency_Total_NoAd, 
    -- SUM(CASE WHEN Count_Offerwall > 0 THEN 1 ELSE 0 END) AS Frequency_Offerwall,
    -- SUM(CASE WHEN Count_Network > 0 THEN 1 ELSE 0 END) AS Frequency_Network,
    -- SUM(CASE WHEN Count_INL_Launcher > 0 THEN 1 ELSE 0 END) AS Frequency_INL_Launcher,
    -- SUM(CASE WHEN Count_INL_ETC > 0 THEN 1 ELSE 0 END) AS Frequency_INL_ETC,
    -- SUM(CASE WHEN Count_Dynamic > 0 THEN 1 ELSE 0 END) AS Frequency_Dynamic,
    -- F_u -> 고유 일자로 집계
...
,rfm_table as (
  SELECT 
    User_ID, MAX(Platform) AS Platform, MIN(Reg_Dttm) AS Reg_Dttm, 
    MAX(Cum_Lifetime) AS Cum_Lifetime,
    MAX(Cum_Reward) AS Cum_Cost_Recent,
    MIN(Cum_Reward_min) AS Cum_Cost_min,
    CASE
      WHEN (MAX(Cum_Reward) - MIN(Cum_Reward_min)) > 0
      THEN ROUND((MAX(Cum_Reward) - MIN(Cum_Reward_min)) / DATE_DIFF(end_date, start_date, DAY), 2)
      ELSE 0
    END AS Avg_Daily_Cost_Period,
    CASE
      WHEN MAX(Cum_Reward) > 0
      THEN ROUND(MAX(Cum_Reward) / DATE_DIFF(DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY), DATE(MIN(Reg_Dttm)), DAY), 2)
      ELSE 0
    END AS Avg_Daily_Cost_Lifetime,
    ROUND(SUM(Rev_Offer_Pincrux) + SUM(Rev_Offer_etc), 2) AS Monatary_Offerwall,
    ROUND(SUM(Rev_Network), 2) AS Monatary_Network,
    ROUND(SUM(Rev_INL_Launcher), 2) AS Monatary_INL_Launcher,
    ROUND(SUM(Rev_INL_Challenge), 2) AS Monatary_INL_Challenge,
    ROUND(SUM(Rev_INL_Quiz), 2) AS Monatary_INL_Quiz,
    ROUND(SUM(Rev_Dynamic), 2) AS Monatary_Dynamic,
    ROUND(SUM(Rev_Offer_Pincrux) + SUM(Rev_Offer_etc) + SUM(Rev_Network) + SUM(Rev_INL_Launcher) + SUM(Rev_INL_Quiz) + SUM(Rev_INL_Challenge) + SUM(Rev_Dynamic), 2) AS Monatary_Total,  # <- 새로운 매출원 Input 
    MIN(Recency_Offerwall) AS Recency_Offerwall,
    MIN(Recency_Network) AS Recency_Network,
    MIN(Recency_INL_Launcher) AS Recency_INL_Launcher,
    MIN(Recency_INL_Challenge) AS Recency_INL_Challenge,
    MIN(Recency_INL_Quiz) AS Recency_INL_Quiz,
    MIN(Recency_Dynamic) AS Recency_Dynamic,
    MIN(Recncy_Total) AS Recncy_Total,
    MIN(Recency_Total_NoAd) AS Recency_Total_NoAd,
    # F_u
    SUM(Frequency_Offerwall) AS Frequency_Offerwall,
    SUM(Frequency_Network) AS Frequency_Network,
    SUM(Frequency_INL_Launcher) AS Frequency_INL_Launcher,
    SUM(Frequency_INL_Challenge) AS Frequency_INL_Challenge,
    SUM(Frequency_INL_Quiz) AS Frequency_INL_Quiz,
    SUM(Frequency_Dynamic) AS Frequency_Dynamic,
    (SUM(Frequency_Offerwall) + SUM(Frequency_Network) + SUM(Frequency_INL_Launcher) + SUM(Frequency_INL_Challenge) + SUM(Frequency_INL_Quiz) + SUM(Frequency_Dynamic)) AS Frequency_Total,
    (SUM(Frequency_Offerwall) + SUM(Frequency_INL_Launcher) + SUM(Frequency_INL_Challenge) + SUM(Frequency_INL_Quiz) + SUM(Frequency_Dynamic)) AS Frequency_Total_NoAd,
    # F_c
    SUM(Frequency_Offerwall_c) AS Frequency_Offerwall_c,
    SUM(Frequency_Network_c) AS Frequency_Network_c,
    SUM(Frequency_INL_Launcher_c) AS Frequency_INL_Launcher_c,
    SUM(Frequency_INL_Challenge_c) AS Frequency_INL_Challenge_c,
    SUM(Frequency_INL_Quiz_c) AS Frequency_INL_Quiz_c,
    SUM(Frequency_Dynamic_c) AS Frequency_Dynamic_c,
    (SUM(Frequency_Offerwall_c) + SUM(Frequency_Network_c) + SUM(Frequency_INL_Launcher_c) + SUM(Frequency_INL_Challenge_c) + SUM(Frequency_INL_Quiz_c) + SUM(Frequency_Dynamic_c)) AS Frequency_Total_c,
    (SUM(Frequency_Offerwall_c) + SUM(Frequency_INL_Launcher_c) + SUM(Frequency_INL_Challenge_c) + SUM(Frequency_INL_Quiz_c) + SUM(Frequency_Dynamic_c)) AS Frequency_Total_NoAd_c,
  FROM rfm_features
  GROUP BY User_ID
  )

,filtered_rfm_table_noad as (
  select User_ID, Cum_Lifetime, Cum_Cost_Recent as Cum_Cost, Cum_Cost_Recent - Cum_Cost_min as Cost_Duration, Avg_Daily_Cost_Period, Avg_Daily_Cost_Lifetime,
  Recency_Total_NoAd as R, Frequency_Total_NoAd as F_u, Frequency_Total_NoAd_c as F_c, Monatary_Total as M, 
  from rfm_table
)

-- select *
-- from filtered_rfm_table_noad

-- select * from rfm_table 
# -> 1007 F_s 수정 완료. 이후 cte 에서 정상적으로 정리되는지 확인 필요

# 10/09 이래로, 콘텐츠 보상 기준 추가하여 분석
select rfm_table.*, c.* except (User_ID)
from rfm_table
left join contents_table c on c.User_ID = rfm_table.User_ID
where Frequency_Dynamic > 0 or Frequency_INL_Quiz > 0 or Frequency_INL_Challenge > 0 or Frequency_INL_Launcher > 0