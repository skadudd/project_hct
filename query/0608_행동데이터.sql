...
WHERE 
  Event_Category IN (
      ...
      'view_get__page (App)', 'Ad Click (App)', 'Ad Impression (App)', 'Spend Credits (App)',
      'tap_try__bf_quiz_join (App)', 'tap_try__bf_friend_get_link (App)', 'tap_try__bf_friend_put_send (App)', 'tap_try__bf_fund_receive (App)', 
      'tap_try__os_launcher (App)', 'tap_try__os_launcher_open (App)'
...
#### 행동 CTE 정의
# 각 피쳐 CTE 정의
rwd_total as (select Airbridge_Device_ID, sum(count) as reward_total
from v
where Event_Category IN ("view_get__reward_done (App)", "tap_get__reward_done (App)")
group by Airbridge_Device_ID, Event_Category),

cnt_total_adImpression As (select Airbridge_Device_ID, sum(count) as adImpression_total
from v
where Event_Category = "Ad Impression (App)"
group by Airbridge_Device_ID, Event_Category),

cnt_total_adClick As (select Airbridge_Device_ID, sum(count) as adClick_total
from v
...
#신규
cnt_quiz AS (
  SELECT Airbridge_Device_ID, SUM(count) AS quiz_count
  FROM v
  WHERE Event_Category = "tap_try__bf_quiz_join (App)"
  GROUP BY Airbridge_Device_ID
),

cnt_friend_get_link AS (
  SELECT Airbridge_Device_ID, SUM(count) AS friend_get_link_count
  FROM v
  WHERE Event_Category = "tap_try__bf_friend_get_link (App)"
  GROUP BY Airbridge_Device_ID
),

cnt_friend_put_send AS (
  SELECT Airbridge_Device_ID, SUM(count) AS friend_put_send_count
  FROM v
  WHERE Event_Category = "tap_try__bf_friend_put_send (App)"
  GROUP BY Airbridge_Device_ID
),

cnt_fund_receive AS (
  SELECT Airbridge_Device_ID, SUM(count) AS fund_receive_count
  FROM v
  WHERE Event_Category = "tap_try__bf_fund_receive (App)"
  GROUP BY Airbridge_Device_ID
),

#신규
cnt_launcher as (SELECT Airbridge_Device_ID, sum(count) as launcher
from v
where Event_Category in (
  'tap_try__os_launcher (App)', 
  'tap_try__os_launcher_open (App)'
)
group by Airbridge_Device_ID, Event_Category),
...
ad_click AS (
    ...
    FROM 
        v
    WHERE 
        Event_Category IN ('Ad Click (App)')
    GROUP BY 
...
ad_impression AS (
    SELECT 
        Airbridge_Device_ID,
        Event_Category,
        SUM(CASE WHEN label = "홈_챌린지리스트" AND action = "직광고_자체배너" THEN count ELSE 0 END) AS adImp_homeChg_ownedBanner,
        SUM(CASE WHEN label = "홈_챌린지리스트" AND action = "네트워크_애드팝콘" THEN count ELSE 0 END) AS adImp_homeChg_adPopcorn,
        SUM(CASE WHEN label = "홈_챌린지리스트" AND action = "네트워크_애드파이" THEN count ELSE 0 END) AS adImp_homeChg_adPie,
        SUM(CASE WHEN label = "홈_챌린지리스트" AND action = "네트워크_모비온" THEN count ELSE 0 END) AS adImp_homeChg_mobion,
        SUM(CASE WHEN label = "홈_상단_캐시버튼" AND action = "직광고_쿠팡" THEN count ELSE 0 END) AS adImp_homeCash_coupang,
        SUM(CASE WHEN label LIKE "%캐시버튼%" AND action = "네트워크_애드팝콘" THEN count ELSE 0 END) AS adImp_homeCash_adPopcorn,
        SUM(CASE WHEN label = "홈_상단_캐시버튼" AND action = "네트워크_애드파이" THEN count ELSE 0 END) AS adImp_homeCash_adPie,
        SUM(CASE WHEN label LIKE "홈_상단%" AND action = "네트워크_애드팝콘카카오" THEN count ELSE 0 END) AS adImp_home_adPopcorn,
        SUM(CASE WHEN label = "홈_바텀시트2" AND action = "직광고_자체배너" THEN count ELSE 0 END) AS adImp_homeBottom_owned,
        SUM(CASE WHEN label = "홈_바텀시트2" AND action = "네트워크_애드팝콘" THEN count ELSE 0 END) AS adImp_homeBottom_adPopcorn,
        SUM(CASE WHEN label = "홈_바텀시트2" AND action = "네트워크_애드파이" THEN count ELSE 0 END) AS adImp_homeBottom_adPie,
        SUM(CASE WHEN label = "혜택_상단" AND action = "네트워크_애드팝콘" THEN count ELSE 0 END) AS adImp_benefit_adPopcorn,
        SUM(CASE WHEN label = "챌린지_챌린지모아보기" AND action = "네트워크_애드팝콘" THEN count ELSE 0 END) AS adImp_chgList_adPopcorn,
        SUM(CASE WHEN label = "챌린지_챌린지모아보기" AND action = "네트워크_애드파이" THEN count ELSE 0 END) AS adImp_chgList_adPie,
        SUM(CASE WHEN label = "챌린지_상단" AND action = "직광고_자체배너" THEN count ELSE 0 END) AS adImp_chg_owned,
        SUM(CASE WHEN label = "챌린지_상단" AND action = "네트워크_애드팝콘카카오" THEN count ELSE 0 END) AS adImp_chg_adPopcorn,
        SUM(CASE WHEN label = "전체_상단" AND action = "네트워크_애드팝콘카카오" THEN count ELSE 0 END) AS adImp_info_adPopcorn,
        SUM(CASE WHEN label = "앱 종료" AND action = "네트워크_애드파이" THEN count ELSE 0 END) AS adImp_exit_adPie,
        SUM(CASE WHEN label = "보너스봉투팝업_하단" AND action = "네트워크_애드팝콘" THEN count ELSE 0 END) AS adImp_benefit_bongtu_adPopcorn,
        SUM(CASE WHEN label = "매일버튼누르기챌린지" AND action = "네트워크_애드팝콘" THEN count ELSE 0 END) AS adImp_benefit_button_adPopcorn,
        SUM(CASE WHEN (label = "구매_상단" OR label = "카테고리상세_상단") AND action = "직광고_자체배너" THEN count ELSE 0 END) AS adImp_purchase_owned
    FROM 
        v
    WHERE 
        Event_Category IN ('Ad Impression (App)')
    GROUP BY 
        Airbridge_Device_ID, Event_Category
),
...
SELECT 
    ...
    users.User_Id, users.Joined_Date, users.Platform, users.Sub_Division, users.Channel, users.Campaign, users.First_Cur_Cash, users.Last_Cur_Cash,
    users.Cum_Cash, users.is_EasyLogin, users.is_CashLauncher, users.is_PushAgreed,
    ...
    # 광고 클릭
    COALESCE(adc.adClick_homeChg_ownedBanner,0) as adClick_homeChg_ownedBanner,
    COALESCE(adc.adClick_homeChg_adPopcorn,0) as adClick_homeChg_adPopcorn,
    COALESCE(adc.adClick_homeChg_adPie,0) as adClick_homeChg_adPie,
    COALESCE(adc.adClick_homeChg_mobion,0) as adClick_homeChg_mobion,
    COALESCE(adc.adClick_homeCash_coupang,0) as adClick_homeCash_coupang,
    COALESCE(adc.adClick_homeCash_adPopcorn,0) as adClick_homeCash_adPopcorn,
    COALESCE(adc.adClick_homeCash_adPie,0) as adClick_homeCash_adPie,
    COALESCE(adc.adClick_home_adPopcorn,0) as adClick_home_adPopcorn,
    COALESCE(adc.adClick_homeBottom_owned,0) as adClick_homeBottom_owned,
    COALESCE(adc.adClick_homeBottom_adPopcorn,0) as adClick_homeBottom_adPopcorn,
    COALESCE(adc.adClick_homeBottom_adPie,0) as adClick_homeBottom_adPie,
    COALESCE(adc.adClick_benefit_adPopcorn,0) as adClick_benefit_adPopcorn,
    COALESCE(adc.adClick_purchase_owned,0) as adClick_purchase_owned,
    COALESCE(adc.adClick_chgList_adPopcorn,0) as adClick_chgList_adPopcorn,
    COALESCE(adc.adClick_chgList_adPie,0) as adClick_chgList_adPie,
    COALESCE(adc.adClick_chg_owned,0) as adClick_chg_owned,
    COALESCE(adc.adClick_chg_adPopcorn,0) as adClick_chg_adPopcorn,
    COALESCE(adc.adClick_info_adPopcorn,0) as adClick_info_adPopcorn,
    COALESCE(adc.adClick_exit_adPie,0) as adClick_exit_adPie,
    COALESCE(adc.adClick_benefit_bongtu_adPopcorn,0) as adClick_benefit_bongtu_adPopcorn,
    COALESCE(adc.adClick_benefit_button_adPopcorn,0) as adClick_benefit_button_adPopcorn,
    
    # 광고 조회
    COALESCE(adc.adClick_homeChg_ownedBanner,0) as adImp_homeChg_ownedBanner,
    COALESCE(adc.adClick_homeChg_adPopcorn,0) as adImp_homeChg_adPopcorn,
    COALESCE(adc.adClick_homeChg_adPie,0) as adImp_homeChg_adPie,
    COALESCE(adc.adClick_homeChg_mobion,0) as adImp_homeChg_mobion,
    COALESCE(adc.adClick_homeCash_coupang,0) as adImp_homeCash_coupang,
    COALESCE(adc.adClick_homeCash_adPopcorn,0) as adImp_homeCash_adPopcorn,
    COALESCE(adc.adClick_homeCash_adPie,0) as adImp_homeCash_adPie,
    COALESCE(adc.adClick_home_adPopcorn,0) as adImp_home_adPopcorn,
    COALESCE(adc.adClick_homeBottom_owned,0) as adImp_homeBottom_owned,
    COALESCE(adc.adClick_homeBottom_adPopcorn,0) as adImp_homeBottom_adPopcorn,
    COALESCE(adc.adClick_homeBottom_adPie,0) as adImp_homeBottom_adPie,
    COALESCE(adc.adClick_benefit_adPopcorn,0) as adImp_benefit_adPopcorn,
    COALESCE(adc.adClick_purchase_owned,0) as adImp_purchase_owned,
    COALESCE(adc.adClick_chgList_adPopcorn,0) as adImp_chgList_adPopcorn,
    COALESCE(adc.adClick_chgList_adPie,0) as adImp_chgList_adPie,
    COALESCE(adc.adClick_chg_owned,0) as adImp_chg_owned,
    COALESCE(adc.adClick_chg_adPopcorn,0) as adImp_chg_adPopcorn,
    COALESCE(adc.adClick_info_adPopcorn,0) as adImp_info_adPopcorn,
    COALESCE(adc.adClick_exit_adPie,0) as adImp_exit_adPie,
    COALESCE(adc.adClick_benefit_bongtu_adPopcorn,0) as adImp_benefit_bongtu_adPopcorn,
    COALESCE(adc.adClick_benefit_button_adPopcorn,0) as adImp_benefit_button_adPopcorn,


    COALESCE(eq.quiz_count, 0) AS quiz_count,
    COALESCE(efgl.friend_get_link_count, 0) AS friend_get_link_count,
    COALESCE(efps.friend_put_send_count, 0) AS friend_put_send_count,
    COALESCE(efr.fund_receive_count, 0) AS fund_receive_count,
    COALESCE(l.launcher, 0) AS launcher
...
LEFT JOIN 
    ad_click AS adc ON v.Airbridge_Device_ID = adc.Airbridge_Device_ID
LEFT JOIN 
    ad_impression AS adi ON v.Airbridge_Device_ID = adi.Airbridge_Device_ID
LEFT JOIN 
...
LEFT JOIN
    cnt_quiz AS eq ON v.Airbridge_Device_ID = eq.Airbridge_Device_ID
LEFT JOIN
    cnt_friend_get_link AS efgl ON v.Airbridge_Device_ID = efgl.Airbridge_Device_ID
LEFT JOIN
    cnt_friend_put_send AS efps ON v.Airbridge_Device_ID = efps.Airbridge_Device_ID
LEFT JOIN
    cnt_fund_receive AS efr ON v.Airbridge_Device_ID = efr.Airbridge_Device_ID
LEFT JOIN
    cnt_launcher AS l ON v.Airbridge_Device_ID = l.Airbridge_Device_ID
